...
export class Scenery {
    ...
    _segment(start) {
        ...
        for (let i = 0; i < 5; i++) {
            if (rng() < 0.3) {
                ...
                for (let j = 0; j < n; j++) {
                    ...
                    rocks.push(new Rock(x, this.terrain.sampleY(x), rng() * 15 + 5, rng));
                }
            } else {
                const x = start + rng() * SEGMENT;
                rocks.push(new Rock(x, this.terrain.sampleY(x), rng() * 10 + 5, rng));
            }
        }

        for (let i = 0; i < 3; i++) {
            const x = start + rng() * SEGMENT;
            trees.push(new Tree(x, this.terrain.sampleY(x), rng() * 40 + 20, rng));
        }
        ...
        for (let j = 0; j < fn; j++) {
            const x = fx + rng() * fw;
            trees.push(new Tree(x, this.terrain.sampleY(x), rng() * 30 + 20, rng));
        }
